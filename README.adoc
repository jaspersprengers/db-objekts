...
This feature is planned for the 0.3.0-alpha release. See the  <<Status and roadmap>> for details.
...
==== 0.3.0-alpha
* Support for aggregates in select clauses: sum(), avg(), count(), which will look like:
```kotlin
    // show the average salary of employees grouped by town
    tr.select(Employee.salary.average, Address.town).asList()
    // show all employee names who have more than two certificates
    tr.select(Employee.name, Certificate.id.sum).where(Certificate.id.sum.gt(2))
```
Each numeric column can be used with a `count`, `min`, `max`, `sum` and `avg` extension, which turns the normal select query in an aggregate one.

The API will only cover common cases, where grouping is done over all selected columns. A `having` clause can be expressed in the `where()` call using normal syntax by referring to the aggregated column.
