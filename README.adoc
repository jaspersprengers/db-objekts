...
This section deals with scenarios where this automatic behavior is not possible or not desirable. When it comes to join syntax, there are three options:
...
==== Automatic join discovery with outer joins
...
==== The semi-manual join chain
The previous query could also be written as follows:
```
tr.select(Employee.name, Hobby.name).from(Employee.leftJoin(Hobby))
```
The `join()` method takes a `JoinChain`, which is created from a `Table` object and at least one call to `innerJoin()`, `leftJoin()` or `rightJoin()`. It yields the SQL you would expect:
```sql
...
```
Without this explicit `from` clause and only the `employee` and `country` tables referenced, there's not enough information to join the tables together, so you need to lend a hand.

==== The fully manual join chain
If there is no foreign key relationship between two tables in the metadata, you need to provide the relevant columns yourself. The `on()` clause takes a clause strung together from a column, an operator, and another column. It's the same syntax you will meet in the next section that covers the where clause in detail.
```kotlin
val manualJoin =
    tr.select(Employee, Address.street, EmployeeAddress.kind, Department.name.nullable, Hobby.name.nullable, Certificate.name.nullable)
        .from(Employee
            .innerJoin(ea).on(ea.employeeId.eq(e.id))
            .innerJoin(a).on(ea.addressId.eq(a.id))
            .leftJoin(h).on(e.hobbyId.eq(h.id))
            .leftJoin(EmployeeDepartment).on(e.id.eq(ed.employeeId))
            .leftJoin(Department).on(ed.departmentId.eq(d.id))
            .leftJoin(Certificate).on(c1.employeeId.eq(e.id))).asList()
