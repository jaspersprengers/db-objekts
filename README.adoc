...
You can supply individual columns or an entire table in the select clause, as you would in SQL: `SELECT book.title, author.*`. For each `Table` metadata object there is a stateful, immutable entity class (`AuthorRow`) which contains the values of an author row and is returned in the Tuple. You will later see how you can use these same entities in update and insert statements.
