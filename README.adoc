...
```kotlin
val customResult: List<Tuple3<String, Boolean?, String>> = transaction
    .select(Employee.name, Employee.married, Address.street)
    .where(Employee.dateOfBirth.lt(LocalDate.of(1980, 1, 1))
        .and(EmployeeAddress.kind.eq(AddressType.HOME)))
    .asList()
```
and use the full power and convenience of your IDE's auto-complete to build type-safe queries.
...
The final step before we can make queries is to configure a `TransactionManager`, which wraps a `javax.sql.DataSource`. It also needs the `CatalogDefinition` that was just generated.
...
Note that `Author.bio` is a nullable column, so it produces a `String?`, not `String` value.
