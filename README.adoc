...
Columns with a small, relatively immutable range of values can be directly retrieved as a Kotlin enum, provided the values match by name or ordinal number (zero-based). Since you're dealing with raw SQL, you must specify the type (integer or character) that db-Objekts should retrieve from the result rows.
