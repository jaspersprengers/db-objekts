...
The code generation utility is at the heart of dbo.

==== Code generation and validation during the SDLC
Generating code from your database is fairly harmless when you're starting out with dbo, but it gets more involved when you have already written production code against the generated metadata objects and the database evolves. You will need to rerun the process, which _could_ lead to compile errors.
