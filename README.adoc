...
db-Objekts is a Kotlin library to query relational databases. It uses generated metadata objects that represent the tables, columns and foreign keys. Its DSL produces type-safe and fluent queries while the engine takes care of join syntax and type conversions. The project was conceived by https://jaspersprengers.eu[Jasper Sprengers]. I am a software developer living in the Netherlands.
...
In striking a balance between feature-completeness and usability however, db-Objekts favours the latter. It tries to cover 80% of query needs with 20% of the complexity. It does not emulate all you can do in native SQL, although it facilitates xref:queries-customsql[native queries] as well.
