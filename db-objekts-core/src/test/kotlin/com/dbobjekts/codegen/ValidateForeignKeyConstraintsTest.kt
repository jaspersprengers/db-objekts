...
class ValidateForeignKeyConstraintsTest {
    ...
    val hobbiesToPeople =
        DBForeignKeyDefinition(hr, hobbyName, ColumnName("people_id"), ColumnFactory.FOREIGN_KEY_LONG, "INT", hr, peopleName, ColumnName("id"))
    val wagesToPeople =
        DBForeignKeyDefinition(hr, wagesName, ColumnName("people_id"), ColumnFactory.FOREIGN_KEY_LONG, "INT", hr, peopleName, ColumnName("id"))

    val peopleTable: DBTableDefinition = DBTableDefinition(pkg, hr, peopleName, "p", listOf(), listOf())
    val hobbiesTable: DBTableDefinition = DBTableDefinition(pkg, hr, hobbyName, "h", listOf(hobbiesToPeople), listOf())
    val wagesTable: DBTableDefinition = DBTableDefinition(pkg, finance, wagesName, "w", listOf(wagesToPeople), listOf())
    ...
    fun `When all tables are included validation is ok`() {
        ...
        val catalog = DBCatalogDefinition(pkg, H2Vendor, listOf(hrSchema, financeSchema), "CatalogDefinition")
