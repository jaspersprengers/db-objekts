...
object ColumnFactory {
    ...
    internal fun integerColumn(nullable: Boolean = false) = if (nullable) INTEGER_NIL else INTEGER
    internal fun shortColumn(nullable: Boolean = false) = if (nullable) SHORT_NIL else SHORT
    internal fun byteArrayColumn(nullable: Boolean = false) = if (nullable) BYTE_ARRAY_NIL else BYTE_ARRAY
    internal fun blobColumn(nullable: Boolean = false) = if (nullable) BLOB_NIL else BLOB
    internal fun clobColumn(nullable: Boolean = false) = if (nullable) CLOB_NIL else CLOB
    internal fun byteColumn(nullable: Boolean = false) = if (nullable) BYTE_NIL else BYTE
    internal fun booleanColumn(nullable: Boolean = false) = if (nullable) BOOLEAN_NIL else BOOLEAN
    internal fun numberAsBooleanColumn(nullable: Boolean = false) = if (nullable) NUMBER_AS_BOOLEAN_NIL else NUMBER_AS_BOOLEAN
    internal fun doubleColumn(nullable: Boolean = false) = if (nullable) DOUBLE_NIL else DOUBLE
    internal fun floatColumn(nullable: Boolean = false) = if (nullable) FLOAT_NIL else FLOAT
    internal fun bigDecimalColumn(nullable: Boolean = false) = if (nullable) BIGDECIMAL_NIL else BIGDECIMAL
    internal fun dateColumn(nullable: Boolean = false) = if (nullable) DATE_NIL else DATE
    internal fun dateTimeColumn(nullable: Boolean = false) = if (nullable) DATETIME_NIL else DATETIME
    internal fun timeColumn(nullable: Boolean = false) = if (nullable) TIME_NIL else TIME
    internal fun timeStampColumn(nullable: Boolean = false) = if (nullable) TIMESTAMP_NIL else TIMESTAMP
    internal fun offsetDateTimeColumn(nullable: Boolean = false) = if (nullable) OFFSET_DATETIME_NIL else OFFSET_DATETIME

    internal fun <C : NonNullableColumn<*>> forClass(clz: Class<C>): C {
    ...
    internal fun <T> getColumnForValue(value: T): Column<T> =
        when (value) {
            is Int -> INTEGER
            is Double -> DOUBLE
            is Long -> LONG
